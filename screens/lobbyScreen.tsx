...
const LobbyScreen = () => {
    ...
    return (
        <Container>
            ...
            <TouchableOpacity onPress={handleCreateGame} style={{ marginBottom: 10 }}><Text>Create Game</Text></TouchableOpacity>
            <GameList>
                {games.map(game => (
                    <GameListItem no_of_players={game.player2Id === null ? 1 : 2} status={game.status} id={game.id} key={game.id} onPress={() => {
                        handleJoinGame(game.id, game.status)
                    }} />
                ))}
            </GameList>
        </Container>
